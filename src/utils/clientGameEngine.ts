...
export const CLIENT_DICTIONARY: Record<string, DictItem[]> = {
  '가': [
...
export function findClientMeaning(word: string): string | null {
  for (const key of Object.keys(CLIENT_DICTIONARY)) {
    const found = CLIENT_DICTIONARY[key].find((item) => item.word === word);
    if (found) return found.meaning;
  }
  return null;
}

export function generateClientComment(userWord: string, aiWord: string): string {
  const hash = Array.from(userWord + aiWord).reduce(
    (acc, char) => acc + char.charCodeAt(0),
    0
  );
  const comments = [
    `'${userWord}'(이)라는 단어가 멋지네요! 저는 '${aiWord}'(으)로 이어서 받습니다.`,
    `'${userWord}'의 마지막 글자를 이어받아 '${aiWord}'(으)로 응수합니다!`,
    `훌륭한 단어 '${userWord}'입니다! 제 답은 바로 '${aiWord}'입니다.`,
    `'${userWord}'을(를) 선택하셨군요! 저는 '${aiWord}'(으)로 계속 진행해볼게요.`
  ];
  return comments[hash % comments.length];
}

export function processClientWordChain(
  userWord: string,
  previousWord: string,
  historyWords: string[],
  ...
  const trimmed = userWord.trim();

  // 1. Basic Korean regex check
  ...
  // 4. History duplicate check
  if (historyWords.includes(trimmed)) {
    return { valid: false, errorReason: '이미 사용된 단어입니다.' };
  }

  // User word is valid! Prepare AI response.
  const userMeaning = findClientMeaning(trimmed);
  ...
  let candidates: DictItem[] = [];
  for (const startSyl of validAiStarts) {
    if (CLIENT_DICTIONARY[startSyl]) {
      candidates.push(...CLIENT_DICTIONARY[startSyl]);
    }
  }

  // Exclude used words & length mode
  ...
  if (candidates.length === 0) {
    // Dynamic generation if no dictionary match: create a plausible Korean word if possible or give up
    // Generate a fallback candidate if dictionary doesn't have one
    const randomEndSyls = ['기', '자', '구', '가', '선', '물', '상', '화', '림', '정'];
    for (const startSyl of validAiStarts) {
      for (const endSyl of randomEndSyls) {
  ...
  const comment = generateClientComment(trimmed, chosen.word);
...
export function getClientHints(
  ...
  for (const syl of validStarts) {
    if (CLIENT_DICTIONARY[syl]) {
      candidates.push(...CLIENT_DICTIONARY[syl]);
    }
  }
