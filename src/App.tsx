...
import { processFallbackWordChain, getLocalHints } from './utils/clientFallback';
...
export default function App() {
  ...
  const handleSubmitWord = async (userWord: string) => {
    ...
    try {
      ...
      console.warn('Backend API request failed, switching to client fallback:', err);
    }

    // Fallback if API was unreachable or failed
    if (!data || typeof data.valid !== 'boolean') {
      data = processFallbackWordChain(userWord, lastWord, historyWords, difficulty, rules);
    }
  ...
  const handleRequestHint = async () => {
    ...
    try {
      ...
      console.warn('Hint API failed, using local fallback:', err);
    }

    setHints(getLocalHints(lastChar, rules.allowInitialSoundRule, historyWords));
    setIsRequestingHint(false);
  };
